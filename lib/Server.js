	...
	constructor(options = {}) {
		...
		this.wsfe.on('command', (client, cmd, params) => {
			if(cmd=='toggle-demo-switch') {
				// Send everyone else the update
				this.toggleSwitchState = params.state
				this.wsfe.sendAll(cmd, params, client.endpoint)
			}
		})

		this.wsfe.on('connect', (client) => {
			// Set the state on initial connect
			this.wsfe.send(client, 'toggle-demo-switch', { state: this.toggleSwitchState })
		})
